...
import torch
from scipy.linalg import fractional_matrix_power, inv,pinv
...
#def compute_ppr(graph: nx.Graph, alpha=0.2, self_loop=True):
    ...
    d = np.diag(np.sum(a, 1))                                     # D^ = Sigma A^_ii  d:ndarry
    dinv = fractional_matrix_power(d, -0.5)                       # D^(-1/2)
    at = np.matmul(np.matmul(dinv, a), dinv)                      # A~ = D^(-1/2) x A^ x D^(-1/2)
    diff=alpha * inv((np.eye(a.shape[0]) - (1 - alpha) * at))   # a(I_n-(1-a)A~)^-1
    return diff

def compute_ppr1(a, alpha=0.1, self_loop=True):
#def compute_ppr(graph: nx.Graph, alpha=0.2, self_loop=True):
    #a = nx.convert_matrix.to_numpy_array(graph)
    if self_loop:
        a = a + np.eye(a.shape[0])                               # A^ = A + I_n
    ...
    dinv = fractional_matrix_power(d, -0.5)                       # D^(-1/2)
    at = np.matmul(np.matmul(dinv, a), dinv)   # A~ = D^(-1/2) x A^ x D^(-1/2)
    diff1= 1/(alpha * inv((np.eye(a.shape[0]) - (1 - alpha) * at)) ) # a(I_n-(1-a)A~)^-1
    return diff1
